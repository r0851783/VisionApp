...
export default function DataScreen({ navigation, route }: DataScreenProps) {
  ...
  const validateDates = () => {
    if (selectedOption === 'Intrede' && !intredeDatum) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bodyHigher}>
        ...
        <Text style={[styles.nameButtonText, { fontWeight: 'bold', fontSize: 20 }]}>
          Datum {selectedOption === 'Intrede' ? 'intrede' : 'uittrede'}
        </Text>
        <TextInput
          style={styles.nameButton}
          placeholder="DD-MM-YYYY"
          value={selectedOption === 'Intrede' ? intredeDatum : uittredeDatum}
          onChangeText={selectedOption === 'Intrede' ? handleIntredeChange : handleUittredeChange}
          keyboardType="default"
        />
